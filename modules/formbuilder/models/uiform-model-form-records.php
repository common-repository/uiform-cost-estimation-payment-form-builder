...
class Uiform_Model_Form_Records {
    ...
    function getFormDataById($id_rec){
        $query = sprintf('select  f.fmb_name,f.fmb_data,frec.fbh_total_amount
        from %s frec
    ...
    function getNameInvoiceField($id_field) {
        ...
        join %s fm on fm.fmb_id=f.form_fmb_id
        join %s rc on rc.form_fmb_id=fm.fmb_id
        where rc.fbh_id=%s and t.fby_id in (8,9,10,11,16,18,39,40,41,42)', $this->tbformfields, $this->tbformtype, $this->tbform, $this->table, $id_field);
        return $this->wpdb->get_results($query);
    }
