...
class Uiform_Fb_Controller_Forms extends Uiform_Base_Module {
    ...
    public function shortcode_uifm_symbol($atts) {
        $symbol='';
        if(isset($this->current_data_main['price_currency_symbol'])){
        $symbol=$this->current_data_main['price_currency_symbol'];    
        }
        ob_start();
         ?>
        <span class="uiform-stickybox-symbol"><?php echo $symbol;?></span>
        <?php
    ...
    public function shortcode_uifm_currency($atts) {
        $currency='';
        if(isset($this->current_data_main['price_currency'])){
        $currency=$this->current_data_main['price_currency'];    
        }
        ob_start();
         ?>
        <span class="uiform-stickybox-currency"><?php echo $currency;?></span>
        <?php
        $str_output=ob_get_contents();
        ob_end_clean();
        return $str_output;
        ...
        ob_start();
         ...
         <span class="uiform-stickybox-summary">
             ...
             <span class="uiform-stickybox-summary-list">
                ...
                <input type="hidden" class="_rockfm_stickybox_summ_rows" value="<?php echo $vars['rows'];?>">                
         </span>               
        <?php
        $str_output=ob_get_contents();
        ob_end_clean();
    ...
    public function shortcode_uifm_summary_link($atts) {
        ...
        ob_start();
         ?>
             <span class="uiform-stickybox-summary-link"><a href="javascript:void(0);" onclick="javascript:rocketfm.costest_summbox_linkPopUp(this);"><?php echo $vars['value'];?></a></span>
        <?php
        $str_output=ob_get_contents();
        ob_end_clean();
            ...
            <div class="uiform-field-wrap uiform-field-move">
                 <div class="uifm-input31-wrap">
                    ...
                    ?>
                           ...
                           <?php echo self::render_template('formbuilder/views/fields/templates/prevpanel_quickopts3.php',array(), 'always');?>
                    </div>
                    </div>
                    <?php
                    $str_output.=ob_get_contents();
    ...
    public function list_uiforms() {
        ...
        //list all forms
        $data = $config = array();
        $config['base_url'] = admin_url() . '?page=uiform_cost_estimate&mod=formbuilder&controller=forms&action=list_uiforms';
